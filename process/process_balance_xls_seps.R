...
ejecutar_proceso <- function(fechaConsulta, configuracion, instituciones) {
  ...
  if (length(linkSegmento) > 0) {
    ...
    info(logger, paste("Enlace de desacarga del archivo:", link))
    ...
    tabla <- tabla[1:(length(tabla)-2)] # Para el archivo S1 se elimina las dos ultimas columnas ya que son totales
